...
w,costs = stochastic_gradient(S_train,label_train,T,r,threshold)
...
plt.ylabel('Cost (J)')
plt.grid()
plt.savefig('costs_stochastic.png')

plt.figure()
plt.plot(S_train.dot(w),label='stochastic')
plt.plot(label_train,label='true')
plt.grid()
plt.legend()
plt.savefig('result_stochastic.png')

...
def bias_variance_decomposition(Data,label,varDict,attributes,iteration,num_sample,t,att_subsets = None):
    ...
    for ind in range(iteration):
        ...
        single_label = _label_modification(prediction(Ensemble[0]['Tree'],Data,varDict))
        ensemble_label = _label_modification(predict_bagging(Data,label,Ensemble,varDict))
    ...
    for ind,single_row in enumerate(single_labels):
        ...
        single_variances.append(np.var(single_avg))
        ensemble_variances.append(np.var(ensemble_avg))
